 ...
 * @returns {Array}
 */
HtmlDiff.prototype.tokenize = function (html, callback) {
    modifyHtmlAccordingToOptions(html, htmlDifferOptions, function (html) {
        callback(_.filter(html.split(/({{.+?}}(?!})|[{}\(\)\[\]#\*`=:;,.<>"'\/]|\s+)/)));
    });
};
...
HtmlDiffer.prototype.diffHtml = function (html1, html2) {
    var htmlDiffer = new HtmlDiff(htmlDifferOptions),
